...
- (void)viewDidLoad {
    ...
    self.printLogQueue = dispatch_queue_create("com.zmqpublisher.pringlogqueue", DISPATCH_QUEUE_SERIAL);
}
-(IBAction)backBtnAction:(id)sender{
    if ([self.delegate respondsToSelector:@selector(eventFromSubViewController:)]) {
        [self.delegate eventFromSubViewController:@{@"name":@"zmqpublisher",@"data":@"panel"}];
    }
}
-(IBAction)startBtnAction:(id)sender{
    if ([startBtn.title isEqualToString:@"Start"]) {
        ...
        [NSThread detachNewThreadWithBlock:^{
            ...
            BOOL status = [self.publisher initPublisherWithUrl:url];
...
-(IBAction)sendBtnAction:(id)sender{
    ...
    [self.publisher publishMessage:cmd];
